...
- Dashboard: `http://<homelab-ip>/`
- Control panel: `http://<homelab-ip>/admin` — enter any username and
...
starts the app at `http://127.0.0.1:80/` (admin at `/admin`). Ctrl+C to
...
6. Sets the timezone to **Asia/Damascus** and enables NTP time sync
   ...
   sync
7. Installs and enables **ufw**, default-deny incoming, opens SSH (22),
   the HTTP dashboard port (80), and the HTTPS admin port (443) —
   optionally restricted to a LAN subnet you specify
...
`http://localhost/` never has to deal with a certificate warning.

- First visit to `https://<pi-hostname-or-ip>/admin`: your browser will
